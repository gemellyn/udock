...
class NYCamera
{
	public:
		...
		NYCamera() :  _Position(0,-1,0), _LookAt(0,0,0), _UpRef(0,0,1),_UpVec(0,0,1)
		{
			updateVecs();
		}
		...
		{
			NYVert3Df previousLAt = _LookAt;

			_LookAt -= _Position;
			_LookAt.rotate(_NormVec,angle);
			_LookAt += _Position;

			NYVert3Df nrmDir = (_LookAt - _Position).normalize();

			float newAngle = nrmDir.scalProd(_UpRef);
