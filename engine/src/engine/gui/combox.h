...
class GUIComboBox : public GUIPanel
{
	...
	public:
		...
		void setColorBorder(NYColor & color)
		{
			LstBox->ColorBorder = color;
			EdtBox->ColorBorder = color;
		}

		void setColorFond(NYColor & color)
		{
			LstBox->ColorFond = color;
			EdtBox->ColorFond = color;
		}
		...
		void setPos(uint16 x,uint16 y,uint16 width,uint16 nbElementsAff)
		{
			...
			LstBox->setPos(X,Y + EdtBox->Height + 5,Width,LstBox->Height);
