	...
	public:
		...
		void render(uint16 zorder)
		{
			if(Visible && zorder == ZOrder)
			{
				...
				uint16 lenAff = _TextEngine->stringSize(Titre);
				//lenAff = Titre.length() * _TextEngine->fontWidth();
				glColor4f(ColorBorder.R, ColorBorder.V, ColorBorder.B,1.0f);
				//glRasterPos2i(X+(Width-lenAff)/2,Y + (Height+up)/2);
				_TextEngine->glSetTextPos(X+(Width-lenAff)/2,Y + (Height+up)/2);
				//_TextEngine->glSetTextPos(X,Y);
