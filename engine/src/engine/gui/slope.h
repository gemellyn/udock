...
class GUISlope : public GUIPanel
{
	...
	public:
		...
		uint16 addCourbe(const NYColor & color)
		{
		...
		void render(uint16 zorder)
		{
			if(Visible && zorder == ZOrder)
			{
				...
				_TextEngine->glSetTextPos(X+(Width-lenAff)/2,(int) (Y + Height - (MargeY/2-up)/2));
				_TextEngine->glPrint(_Coords.length(),_Coords.c_str());
				lenAff = Titre.length() * _TextEngine->fontWidth();
				_TextEngine->glSetTextPos(X+(Width-lenAff)/2,(int) (Y + (MargeY/2-up)/2));
