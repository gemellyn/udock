...
class GUIPanel
{
	...
	public:
		...
		static void inline drawSquare(uint32 x, uint32 y, uint32 width, uint32 height, NYColor color)
		{
		...
		static void inline drawSquareFilled(uint32 x, uint32 y, uint32 width, uint32 height, NYColor color)
		{
			glColor4f(color.R,color.V,color.B,color.A);
			glBindTexture(GL_TEXTURE_2D, 0);
			glBegin(GL_QUADS);
