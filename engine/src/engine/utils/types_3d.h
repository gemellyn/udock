...
public:
	...
	bool operator== (const NYPoint3D & point) const
	{
		return (X == point.X &&
		Y == point.Y &&
		Z == point.Z);
	...
	bool operator< (const NYPoint3D & point) const //pas de sens mais utile pour mettre dans une map
	{
		return !(*this == point);
	}
};
...
class NYVert2Df
{
	public:
		...
		NYVert2Df (float x, float y) : X(x), Y(y)
		{
			X = x;
			Y = y;
		}
...
class NYVert4Df
{
	public:
		...
		NYVert4Df (float x, float y, float z, float t) : X(x), Y(y), Z(z), T(t)
		{
		}
};
...
class NYVert3Df
{
	public:
		...
		float angleZ(NYVert3Df & vertex)
		{
			...
			NYVert3Df normale = vertex.vecProd(moi);
//			calcNormale(vertex,moi,normale);
...
class NYPlane
{
	...
	public:
		NYPlane() : Xnorm(0),Ynorm(0),Znorm(0),Distance(0)
		{
		}

		NYPlane(double xNorm, double yNorm, double zNorm, double distance) : Xnorm(xNorm),Ynorm(yNorm),Znorm(zNorm),Distance(distance)
		{
		}
...
struct NYFloatMatrix
{
	...
	void createRotateLookatMatrix(NYVert3Df & dir, NYVert3Df & up)
	{
		createIdentite();
		NYVert3Df R1 = dir;
		R1.normalize();
		NYVert3Df R0(R1.vecProd(up));
		R0.normalize();
		NYVert3Df R2(R0.vecProd(R1));
	...
	{
		createIdentite();
		NYVert3Df R2 = (lookat - pos)*-1;///Opengl c'est vers -Z;
		R2.normalize();
	...
	*/
	...
	void createRotateAxe(const NYVert3Df & axe, float angle)
	{
		//Creation de la matrice de rot
		if(angle < 0.001 && angle > -0.001)
			angle = 0;
    ...
    bool readBinary(FILE * fe)
    {
        ...
        for(int i=0;i<16;i++)
            if(fread(&(m[i]),sizeof(float),1,fe) != 1)
                return false;
...
class Positionator
{
	...
	public:
		Positionator() : _Rot(),_Pos(), _Parent(NULL), _TempMatrix()
		{
			_Rot.createIdentite();
		}

		void Rotate(const NYVert3Df & axe, float value)
		{
		...
		void Translate(const NYVert3Df & move)
		{
			_Pos+=move;
		}
		...
		*/
		NYVert3Df Transform(const NYVert3Df & point)
		{
		...
		NYVert3Df invTransform(const NYVert3Df & point)
		{
			NYVert3Df res = point;
			res -= _Pos;
		...
		NYVert3Df getWorldPos(const NYVert3Df & point)
		{
		...
		NYVert3Df getWorldRot(const NYVert3Df & point)
		{
		...
		bool readBinary(FILE * fe)
		{
            if(_Rot.readBinary(fe))
            if(_Pos.readBinary(fe))
                return true;
...
class Rotator
{
	...
	public:
		Rotator() : _AxeX(1,0,0),_AxeY(0,1,0),_AxeZ(0,0,1),_Matrix(),_TempMatrix()
		{
...
class NYQuaternion
{
	public:
		...
		NYQuaternion (float x, float y, float z, float t) : X(x), Y(y), Z(z), T(t)
		{
		}
