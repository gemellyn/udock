#ifndef __MOL_STATE__
#define __MOL_STATE__
...
class MolState : public GameState
{
	public:
		...
		int _RenderMode;//Choix du mode de rendu (3d, map1, map2)
		int _ProjType; //Choix du type de projection pour les maps
		bool _FirstPass; //Si c'est la première fois qu'on fait le rendu dans ce mode
		float _SmoothLevel;//A quel point on smoothe la prot
		float _MorphLevel;//A quel point on morphe la prot
		bool _Precise;//Si on fait un smooth precis
		...
		void showEnergie(float energy, float energyCharge)
		{
			...
			_PbarScore->setValue(nymax(0,-energy));
			_PbarCharge->setValue(nymax(0,-energyCharge));
		...
		{
			_PbarBestScore->setMaxMin(nymax(20.0f,-_EnergyHighestScore),0);
			_PbarScore->setMaxMin(nymax(20.0f,-_EnergyHighestScore),0);
			_PbarCharge->setMaxMin(nymax(20.0f,-_EnergyHighestScore),0);
		...
		bool optimizeStep(float * pcent)
		{
			...
			int nbStepsTotal = 0;
			int nbGoodSteps = 0;
			float sizeTranslationStepBase = 0.5f;
			float sizeRotationStepBase = (float)M_PI/25.0f;
			...
			while (elapsed < _DureeStepOptim)
			{
				...
				elapsed = tStart.getElapsedMs(false);

				//Si le déplacement précédent était mauvais
				if(!lastGood)
				{
					sizeTranslationStep = sizeTranslationStepBase * (nymax(0,(_DureeRestanteOptim-elapsed))/_DureeTotaleOptim);
					sizeRotationStep = sizeRotationStepBase *(nymax(0,(_DureeRestanteOptim-elapsed))/_DureeTotaleOptim);
			...
			/*QueryPerformanceCounter(&t2);
			elapsedLong = t2.QuadPart-t1.QuadPart;
			elapsed = (float) ((float)elapsedLong/(float)freq);*/
		...
		void resetPositions(bool randomRot = true, bool reset_harpoons = true)
		{
		...
		bool activate(bool active)
		{
			...
			//Il va falloire recalculer comme pour une première passe
			if(active)
				_FirstPass = true;
			...
			if(active)
			{
				if (_MoleculesToMatch[0]->_Body == NULL)
					_MoleculesToMatch[0]->addToPhysicEngine(_PhysicEngine);
				if (_MoleculesToMatch[1]->_Body == NULL)
					_MoleculesToMatch[1]->addToPhysicEngine(_PhysicEngine);
				...
				((NYCameraAnimated*)(_Renderer->_Camera))->startAnimTo(_Renderer->_Camera->_Position, NYVert3Df(0, 0, 1), NYVert3Df(0, 0, 0), 0.5f);
		...
		void preState(float elapsed)
		{
			...
			dampMols(elapsed, !_Force && _DampMols, _RoateMol ? _SelectedMol : NULL);
		...
		void postState(float elapsed)
		{
			...
			if(!optimizeStep(&pcent))
			{
				...
				if (_EvalElapsed > 0.25 && _ContinuousEval)
				{
					_EvalElapsed -= 0.25;
					evalEnergie();
				}
		...
		void specialDownFunction(int key, int p1, int p2)
		{
			...
			if (key == SDL_SCANCODE_F7)
			{
				_ContinuousEval = !_ContinuousEval;
			}
		...
		void keyboardDownFunction(unsigned char key, int p1, int p2)
		{
			...
			{
				_Renderer->_ClearBuffs = !_Renderer->_ClearBuffs;
			}*/
		}
		...
		void mousePollFunction(float elapsed)
		{
			...
			else
			{
				...
				if(_SelectedMol && _SelectedMol->_Body)
				{
					...
					//_SelectedMol->_Body->applyTorque(NYVert3Df(0,400,0));
		...
		void gamePadFunction(NYVert3Df lj, NYVert3Df rj, float ltrig, float rtrig, bool ljoyb, bool rjoyb, bool lb, bool rb, bool a, bool b, bool x, bool y, bool xbox)
		{
			...
			//select left or right mol
			Molecule * leftMol = _MoleculesToMatch[0];
			Molecule * rightMol = _MoleculesToMatch[1];
			
			float x1 = (_MoleculesToMatch[0]->_Transform._Pos - _Renderer->_Camera->_Position).scalProd(_Renderer->_Camera->_NormVec);
			float x2 = (_MoleculesToMatch[1]->_Transform._Pos - _Renderer->_Camera->_Position).scalProd(_Renderer->_Camera->_NormVec);

			if (x1 > x2)
			{
				leftMol = _MoleculesToMatch[1];
				rightMol = _MoleculesToMatch[0];
			}
			...
			{
				if (_MoleculesToMatch[0]->_Body)
					_MoleculesToMatch[0]->_Body->setFreeze(false);

				if (_MoleculesToMatch[1]->_Body)
					_MoleculesToMatch[1]->_Body->setFreeze(false);
			}
			...
			if (rb)
			{
				mol = rightMol;
				if (leftMol->_Body)
					leftMol->_Body->setFreeze(true);
			}

			if (lj.getSize() > deadMin)
			{
				if (mol)
				{
					_DampMols = false;
					NYVert3Df torque = this->_Renderer->_Camera->_Direction * 1000 * lj.X * 100 * _Renderer->_DeltaTime;
					if (mol->_Body)
			...
			if (rj.getSize() > deadMin)
			{
				...
				if (mol)
				{
					...
					NYVert3Df torqueH = this->_Renderer->_Camera->_UpVec * 1000 * rj.X * 100 * _Renderer->_DeltaTime;
					NYVert3Df torqueV = this->_Renderer->_Camera->_NormVec * 1000 * rj.Y * 100 * _Renderer->_DeltaTime;
					if (mol->_Body)
					{
			...
			if (_Force)
			{
				if (_MoleculesToMatch[0]->_Body && _MoleculesToMatch[1]->_Body)
				{
					NYVert3Df as = _MoleculesToMatch[0]->_Body->getAngularSpeed();
					if (as.getSize() > 5)
					{
						as.normalize();
						as *= 5;
						_MoleculesToMatch[0]->_Body->setAngularSpeed(as);
					}
					as = _MoleculesToMatch[1]->_Body->getAngularSpeed();
					if (as.getSize() > 5)
					{
						as.normalize();
						as *= 5;
						_MoleculesToMatch[1]->_Body->setAngularSpeed(as);
					}
				}
			}
			...
			if (y && !y_last)
				this->resetPositions();
			if (x && !x_last)
				this->resetPositions(false,false);
			if (a && !a_last)
				this->evalEnergie();
