...
public:
	...
	float * _MolVert; // vertices position for mol VBO
	float * _MolNorm; // normals for mol VBO
	float * _MolPola; // polarity for mol VBO
	float * _MolNrgy; // Energy for mol VBO

	//const for VBO offset
	const uint8_t SIZE_POLARITY = 4;
	const uint8_t SIZE_NORMALS = 3;
	const uint8_t SIZE_VERTICES = 3;
	const uint8_t SIZE_ENERGY = 1;

	unsigned int VBO_VerticesCount; // nb of vertices in the VBO
...
public:
	...
	*/
	...
	void makeGeometryFaces(const NYVert3Df & translateOrigin)
	{
		...
		_VertexFloatsCountVBO = 4 + 3 + 3 + 1; //Taille d'un vertex en nombre de floats dans le VBO
		// 4 = polarity color, 3 = normal, 3 = position, 1 = energy score
					...
					while (MCube::TianglesPerCode[offset + i] != -1 && i < 15)
					{
						...
						for (int j = 0; j<3; j++)
						{
							...
							*ptVertices = vertice[2 - j].Z; ptVertices++;
							...
							//energy score initialisation
							*ptVertices = 0.0f; ptVertices++;
						}
	...
	void makeGeometryFacesDataOriented(const NYVert3Df & translateOrigin)
	{
		//On se recalcule les normales si on est en mode interpolation et donc si les edges sont deja calcules
		computeNormals();
		...
		//D'ou le nombre de sommets
		VBO_VerticesCount = nbTriangles * 3;
		...
		_MolNorm = new float[SIZE_NORMALS * VBO_VerticesCount];
		if (_MolNorm == nullptr)
		{
		...
		_MolPola = new float[SIZE_POLARITY * VBO_VerticesCount];
		if (_MolPola == nullptr)
		{
		...
		_MolNrgy = new float[SIZE_ENERGY * VBO_VerticesCount];
		if (_MolNrgy == nullptr)
		{
			Log::log(Log::ENGINE_ERROR, "Memory allocation failed for _MolNrgy of MarchingCubes");
			return;
		}

		//initPointersOnArrays
		float* ptrVert = _MolVert;
		float* ptrNorm = _MolNorm;
		float* ptrPola = _MolPola;
		float* ptrNrgy = _MolNrgy;
					...
					while (MCube::TianglesPerCode[offset + i] != -1 && i < 15)
					{
						...
						for (int j = 0; j<3; j++)
						{
							...
							if (colorShift > 0)
							{
								...
								*ptVertices = COLOR_BASE_B; ptVertices++;*/
								NYColor res = colorBase.interpolate(colorMaxPos, colorShift);
								*ptrPola = res.R; ptrPola++;
								*ptrPola = res.V; ptrPola++;
								*ptrPola = res.B; ptrPola++;
							}
							...
							if (colorShift == 0)
							{
								*ptrPola = COLOR_BASE_R; ptrPola++;
								*ptrPola = COLOR_BASE_V; ptrPola++;
								*ptrPola = COLOR_BASE_B; ptrPola++;
							}

							*ptrPola = 1.0f; ptrPola++;

							//Le bon sens pour GL_CULLFACES

							*ptrNorm = normals[2 - j].X; ptrNorm++;
							*ptrNorm = normals[2 - j].Y; ptrNorm++;
							*ptrNorm = normals[2 - j].Z; ptrNorm++;

							*ptrVert = vertice[2 - j].X; ptrVert++;
							*ptrVert = vertice[2 - j].Y; ptrVert++;
							*ptrVert = vertice[2 - j].Z; ptrVert++;

							//energy score initialisation
							*ptrNrgy = 0.0f; ptrNrgy++;
						}
						i += 3;
					}
				}
		...
		NYRenderer::checkGlError("Failed when creating VBO buffer");
